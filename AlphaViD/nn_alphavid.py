...
class Net(nn.Module):
    def __init__(self, params = Parameters()):
        ...
        # Decoder token head
        self.decoder_token_head = nn.Sequential(
            nn.LayerNorm(self.params.emb_size),
            nn.Linear(self.params.emb_size, self.params.decoder_emb_size),
        )
    ...
    def forward(self, input, game_name, action_size):
        device = input.device
        ...
        # Decoder
        y = x[:,2:]
        y = self.decoder_token_head(y)
        y = y.view(-1, 1, y.shape[1], y.shape[2])
        y = F.interpolate(y, size = (action_size, self.params.decoder_emb_size), mode='bilinear', align_corners=False)
        y = y.view(-1, action_size, self.params.decoder_emb_size)
...
class NNetWrapper:
    ...
    def predict(self, states, game_name):
        ...
        with torch.no_grad():
            # FP16
            with amp.autocast():
                pi, v = self.net(board.unsqueeze(0), game_name, self.params.action_size[game_name])
        ...
        for epoch in range(self.params.epochs):
            ...
            for _ in range(len(train_loaders[games[0]])):
                for game_name in games:
                    ...
                    # out_pis, out_vs = model(boards, game_name, self.params.action_size[game_name])
