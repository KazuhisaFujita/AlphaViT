...
class MCTS:
    ...
    def l(self, q, n, N):
        # UCT score
        return float(q)/(n + random.random()*1e-5) + 0.5 * math.sqrt(2 * math.log(N + 1) / (n + random.random()*1e-5))
    ...
    def Get_prob(self):
        ...
        action_size = self.num**2 +1
        prob = np.zeros(action_size)
        for i in self.root.children:
            prob[i.move[0] * self.num +  i.move[1]] += i.n
        prob /= np.sum(prob)
        return(prob)
