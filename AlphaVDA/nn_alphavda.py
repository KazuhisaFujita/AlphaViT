...
from parameters_alphavda import Parameters
...
class Net(nn.Module):
    def __init__(self, params = Parameters()):
        ...
        # Transformer Decoder
        self.move_embeddings = nn.Parameter(torch.randn(1, 1, self.params.decoder_token_size, self.params.decoder_emb_size))

        self.decoder_layer = nn.TransformerDecoderLayer(
            d_model   = self.params.decoder_emb_size,
            nhead     = self.params.decoder_num_heads,
            activation= 'gelu',
            dropout = self.params.dropout,
            dim_feedforward = self.params.decoder_dim_feedforward,
            batch_first=True,
            norm_first=True
        )
        self.transformer_decoder = nn.TransformerDecoder(self.decoder_layer, num_layers=self.params.decoder_num_layers)

        # Policy head
        self.policy_head = nn.Sequential(
            nn.LayerNorm(self.params.decoder_emb_size),
            nn.Linear(self.params.decoder_emb_size, self.params.num_policy_hidden),
            nn.GELU(),
            nn.Linear(self.params.num_value_hidden, 1)
        )

    def forward(self, input, action_size):
        device = input.device
        ...
        # Decoder
        move_embeddings = F.interpolate(self.move_embeddings, size = (action_size, self.params.decoder_emb_size), mode='bilinear', align_corners=False)
        move_embeddings  = move_embeddings.squeeze(0).squeeze(0)
        y = move_embeddings.to(device).expand(B, -1, -1)

        x = self.dropout(x)
        y = self.dropout(y)
        y = self.transformer_decoder(y, x)
...
class NNetWrapper:
    ...
    def predict(self, states, game_name):
        ...
        with torch.no_grad():
            # FP16
            with amp.autocast():
                pi, v = self.net(board.unsqueeze(0), self.params.action_size[game_name])
        ...
        for epoch in range(self.params.epochs):
            ...
            for _ in range(len(train_loaders[games[0]])):
                for game_name in games:
                    ...
                    # out_pis, out_vs = model(boards, self.params.action_size[game_name])
