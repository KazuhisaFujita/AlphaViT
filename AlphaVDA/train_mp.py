...
from AlphaVDA_mcts import A_MCTS
from collections import deque
from nn_alphavda import NNetWrapper
...
\
            for i in range(self.params.input_size):
                ...
                if self.start == 1:
                    if game_name == "Othello" or game_name == "Othello66":
                        buf_prob[game_name].add(probs[i])
                    elif game_name == "Gomoku" or game_name == "Gomoku77" or game_name == "Gomoku66" or game_name == "TicTacToe":
                        buf_prob[game_name].add(np.delete(probs[i], -1))
                    elif game_name == "Connect4":
                        buf_prob[game_name].add(probs[i][[0,6,12,18,24,30,36]])
                    elif game_name == "Connect4_65":
                        buf_prob[game_name].add(probs[i][[0,5,10,15,20,25]])
                    elif game_name == "Connect4_54":
                        buf_prob[game_name].add(probs[i][[0,4,8,12,16]])
                else:
                    buf_prob[game_name].add(probs[i])
        ...
        for i in range(self.start + 1, self.params.num_iterations+1):
            ...
            for game_name in self.games:
                ...
                if game_name == "Connect4" or game_name == "Connect4_65" or game_name == "Connect4_54":
                    ...
                    temp_board, temp_prob, temp_v = self.Augment_data_gomoku(training_board[game_name], training_prob[game_name], training_v[game_name],
                                                                      self.params.board_x[game_name], self.params.board_y[game_name])
                else:
                    temp_board, temp_prob, temp_v = self.Augment_data_othello(training_board[game_name], training_prob[game_name], training_v[game_name],
                                                                      self.params.board_x[game_name], self.params.board_y[game_name])
